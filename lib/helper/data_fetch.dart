  ...
  switch (searchType) {
    case SearchType.Filtered:
      response = await http.get(
          'https://api.github.com/search/users?q=$searchText&page=$pageIndex&per_page=10');
    ...
    case SearchType.All:
      response = await http
          .get('https://api.github.com/users?since=$sinceIndex&per_page=10');
...
Future<Map> fetchProjects({String userId, int pageIndex}) async {
  List<Project> newProjects = [];
  final response = await http.get(
      'https://api.github.com/users/$userId/repos?page=$pageIndex&per_page=10');
...
Future<GitUser> fetchGitUser(String userId) async {
  final response = await http.get('https://api.github.com/users/$userId');
